odoo.define('pos_orders_all.pos', function(require) {
	...
	models.PosModel = models.PosModel.extend({
		...
		initialize: function (session, attributes) {
			...
			// add wh qty
			product_model.fields.push('available_quantity','qty_available','virtual_available','incoming_qty','outgoing_qty','type','warehouse_quantity');
		...
		init: function(parent, options) {
			...
			this.click_product_handler = function(){
				...
				if(self.pos.config.show_stock_location == 'specific')
				{
					...
					// remove code that prevent cashier to sell if qty negative edit 
...
// Load Models here...
	...
	models.load_models({
		model: 'pos.order',
		fields: ['name', 'id', 'date_order','discount_type', 'partner_id', 'pos_reference', 'lines', 'amount_total','amount_tax','session_id', 'state', 'company_id','coupon_id','f_barcode','ean13'],
		domain: function(self){ 
	...
	var PosBarcodePopupWidget = popups.extend({
		...
		renderElement: function() {
			...
			this.$('#apply_barcode_return_order').click(function() {
					...
					var selectedOrder = null;
					for(var i = 0, len = Math.min(orders.length,1000); i < len; i++) {
						if (orders[i] && orders[i].f_barcode == entered_barcode) {
							selectedOrder = orders[i];
						}
			...
			this.$('#apply_return_order').click(function() {
				...
				$.each(list_of_qty, function(index, value) {
					...
					if(!exact_entered_qty){
					  ...
					  // to view qty
//					  exact_return_qty[qty_id] = entered_qty;
//					  consol.log(entered_qty)
	 ...
	 //        var year = d.getFullYear();
		...
		renderElement: function() {
			...
			this.$('#create_coupon').click(function() {
				...
				else if(max_exp_dt.getTime() < issu_dt.getTime()){
					self.gui.show_popup('create_coupon_popup_widget',{});
					$('#alertdate').show()
				}
