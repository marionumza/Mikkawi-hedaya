...
odoo.define('aar_pos_ticket_receipt.order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        export_as_JSON: function () {
            ...
            if (this.f_barcode) {
                json.f_barcode = this.f_barcode;
            }
            if (!this.f_barcode && this.uid) { // init f_barcode and automatic create f_barcode for order
                var f_barcode = '';
//                if (this.pos.user.id) {
//                    f_barcode += this.pos.user.id;
//                }
//                if (this.sequence_number) {
//                    f_barcode += this.sequence_number;
//                }
//                if (this.pos.config.id) {
//                    f_barcode += this.pos.config.id;
//                }
                
              //  f_barcode = this.pos.pos_session.login_number;
                var fean13 = this.uid.split('-');
                for (var i in fean13) {
                    f_barcode += fean13[i];
                }
                f_barcode = f_barcode.split("");
                var aean13 = []
                var sean13 = ""
                for (var i = 0; i < f_barcode.length; i++) {
                    if (i < 12) {
                        sean13 += f_barcode[i]
                        aean13.push(f_barcode[i])
                    }
                }
                this.f_barcode = sean13 + this.generate_f_barcode(aean13).toString()
            }
            return json;
        },
        generate_f_barcode: function (code) {
            if (code.length != 12) {
                return -1
            }
            var evensum = 0;
